...
from . import collection_pb2 as pb
...
TYPE_NAME_TO_ID = {"controller": 1, "hand": 2, "hand_half":  3,}
TYPE_ID_TO_NAME = {v: k for k, v in TYPE_NAME_TO_ID.items()}

TYPE_TO_PBMSG = {
    "controller": pb.ControllerInput,
    "hand":       pb.HandInput,
    "hand_half":  pb.HandInputV2,
}

# ========= Half 解码工具：bytes -> float32 =========
def _half_vec3_to_dict(hv) -> dict:
    """
    hv: pb.HalfVector3
    hv.packed 应该是 6 字节：x,y,z 各一个 float16，小端。
    返回 {"x": float32, "y": float32, "z": float32}
    """
    b = hv.packed
    if len(b) != 6:
        # 长度异常就按 0 处理，避免崩
        return {"x": 0.0, "y": 0.0, "z": 0.0}
    arr = np.frombuffer(b, dtype=np.float16, count=3)
    return {
        "x": float(arr[0]),
        "y": float(arr[1]),
        "z": float(arr[2]),
...
def _half_quat_to_dict(hq) -> dict:
    """
    hq: pb.HalfQuaternion
    hq.packed 应该是 8 字节：x,y,z,w 各一个 float16
    返回 {"x":..,"y":..,"z":..,"w":..}
    """
    b = hq.packed
    if len(b) != 8:
        return {"x": 0.0, "y": 0.0, "z": 0.0, "w": 1.0}
    arr = np.frombuffer(b, dtype=np.float16, count=4)
    return {
        "x": float(arr[0]),
        "y": float(arr[1]),
        "z": float(arr[2]),
...
def _half_finger_to_dict(hf) -> dict:
    """
    hf: pb.HalfFingerState
    hf.packed 应该是 10 字节：5 个 float16
    顺序约定：fullCurl, baseCurl, tipCurl, pinch, spread
    返回 {"fullCurl":..,"baseCurl":..,"tipCurl":..,"pinch":..,"spread":..}
    """
    b = hf.packed
    if len(b) != 10:
        return {
    ...
    arr = np.frombuffer(b, dtype=np.float16, count=5)
    return {
        "fullCurl": float(arr[0]),
        "baseCurl": float(arr[1]),
        "tipCurl":  float(arr[2]),
        "pinch":    float(arr[3]),
        "spread":   float(arr[4]),
    }

def _one_hand_half_to_dict(h: pb.OneHandHalf) -> dict:
    """
    把 OneHandHalf（Half 精度）还原成和原来 OneHand 基本一致的 dict 结构：
    {
      "isTracked": bool,
      "rootPose": {
        "position": {"x":..,"y":..,"z":..},
      ...
      "fingers": [
        {
          "fullCurl":..,"baseCurl":..,"tipCurl":..,"pinch":..,"spread":..
        },
        ...
      ]
    }
    """
    out = {
        "isTracked": bool(h.isTracked),
        "rootPose": {
            "position": _half_vec3_to_dict(h.rootPose.position),
            "rotation": _half_quat_to_dict(h.rootPose.rotation),
        },
        "joints": [],
        "fingers": [],
    }
    ...
    for j in h.joints:
        out["joints"].append({
            "position": _half_vec3_to_dict(j.position),
            "rotation": _half_quat_to_dict(j.rotation),
        })

    for f in h.fingers:
        out["fingers"].append(_half_finger_to_dict(f))

    return out

def handinputv2_to_float_json(msg: pb.HandInputV2) -> dict:
    """
    HandInputV2 -> dict，结构尽量和原来的 HandInput 一致：
    {
      "leftHand": {...},
      "rightHand": {...}
    }
    """
    return {
        "leftHand":  _one_hand_half_to_dict(msg.leftHand),
        "rightHand": _one_hand_half_to_dict(msg.rightHand),
    }
        ...
        if length >= HDR_SIZE:
            ...
            if magic == MAGIC and ver == PROTO_VER and (valid_varlen or valid_fixed):
                ...
                if mtype == TYPE_SYNC_REQ and plen >= 8:
                    ...
                    _ = self._dll.TRRO_sendControlData(buf, len(frame), 0)
                    return
                ...
                if mtype == MSG_DATA:
                    ...
                    if plen >= 2:
                        ...
                        # print("[RX] kind=", kind, "type_str=", type_str)
                        if type_str and type_str in TYPE_TO_PBMSG:
                            try:
                                if type_str == "hand_half":
                                    # 特殊处理：使用 HandInputV2 + Half 解码成 float32 JSON
                                    msg_hand = pb.HandInputV2()
                                    msg_hand.ParseFromString(pb_bytes)
                                    payload_obj = handinputv2_to_float_json(msg_hand)

                                    # 这里有两个选择：
                                    #  1) 对上层暴露 type="hand"（让上层完全当作老的 hand 在用）
                                    #  2) 暴露 type="hand_half_decoded"
                                    # 这里我们选择 1，兼容你原来的逻辑：
                                    out_type = "hand"
                                else:
                                    ...
                                    out_type = type_str
                                out = {"type": out_type, "payload": payload_obj, "flags": int(flags)}
